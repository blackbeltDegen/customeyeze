...
export default function Testimonials() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Section height = sticky viewport (100vh) + however far the track needs to
      // travel, so the sticky inner wrapper naturally "pins" via CSS instead of
      // ScrollTrigger's DOM-reparenting `pin: true` (see ExplodedProcess for why).
      section.style.height = `${window.innerHeight + distance + window.innerHeight * 0.4}px`;
  ...
  return (
    <section ref={sectionRef} style={{ position: "relative" }}>
      <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden", backgroundColor: "#0C150C", display: "flex", alignItems: "center" }}>
